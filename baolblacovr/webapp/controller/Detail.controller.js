  ...
  function (
    ...
    return BaseController.extend("baolblacovr.controller.Detail", {
      ...
      onInit: function () {
        ...
        this.setModel(new JSONModel(), "visibility");
        ...
        this._fetchConfigurationFields();
      },

      _fetchConfigurationFields: function () {
        this.getOwnerComponent().getModel("utilitiesService").read("/ConfigFieldSet", {
          filters: [new Filter("ExternalServiceName", FilterOperator.EQ, "LB_LACOVR_B1_SRV")],
          success: function (oData) {
            var data = oData.results;
            var oVisibilityModel = this.getModel("visibility");
            for (var i = 0; i < data.length; i++) {
              var fieldName = "/" + data[i].Field;
              oVisibilityModel.setProperty(fieldName, data[i].IsEnabled);
            }
          }.bind(this),
          error: function () { },
        });
