...
sap.ui.define([
	...
	return BaseController.extend("baolblacovr.controller.Master", {
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				if (isNaN(sQuery)) {
					this._oListFilterState.aSearch = [new Filter("BorrowerTxt", FilterOperator.Contains, sQuery)];
				} else {
					this._oListFilterState.aSearch = [new Filter("BorrowerID", FilterOperator.EQ, sQuery)];
				}
			} else {
		...
		onOpenViewSettings: function () {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("baolblacovr.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function (oEvent) {
			...
			aFilterItems.forEach(function (oItem) {
				switch (oItem.getKey()) {
				case "Filter1":
					aFilters.push(new Filter("BankAccount/CurrentBalance", FilterOperator.LE, 100));
					break;
				case "Filter2":
					aFilters.push(new Filter("BankAccount/CurrentBalance", FilterOperator.GT, 100));
					break;
				default:
					break;
				}
		...
		onNavBack: function () {
			...
			if (sPreviousHash !== undefined) {
				...
				var PartnerID = this._FA2A_getPartnerID();
		...
		_FA2A_getPartnerID: function () {
			...
			if (oMyComponent && oMyComponent.getComponentData()) {
				aStartupParameters = oMyComponent.getComponentData().startupParameters;
				if (aStartupParameters) {
					if (aStartupParameters.PartnerID && aStartupParameters.PartnerID[0]) {
						var PartnerID = aStartupParameters.PartnerID[0];
					}
					return PartnerID;
				}
			}
