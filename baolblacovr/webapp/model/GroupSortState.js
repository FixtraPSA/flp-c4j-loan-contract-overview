sap.ui.define([
	...
	return BaseObject.extend("sap.bao.fs.am.overview.model.GroupSortState", {
		 ...
		 * @alias sap.bao.fs.am.overview.model.GroupSortState
		 */
		...
		/**
		 * Sorts by Party/BusinessPartnerFormattedName, or by BankAccount/CurrentBalance
		 *
		...
		/**
		 * Groups by BankAccount/CurrentBalance, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "BankAccount/CurrentBalance") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "BankAccount/CurrentBalance");

				aSorters.push(
					new Sorter("BankAccount/CurrentBalance", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Party/BusinessPartnerFormattedName");
			}
